(function (root) {
  ...
  function resolveModuleEntry(search) {
    ...
    const gatewayRequired = embed && Boolean(params.get("require_gateway"));
    const allowed = standalone || !gatewayRequired || Boolean(token) || Boolean(hostOrigin);
    return {
  ...
  function applyShellChrome(entry) {
    ...
    if (entry.embed && entry.hostOrigin && root.parent && root.parent !== root) {
      root.parent.postMessage(
        { type: "inneros.module.ready", module_id: "alpha.trading.alpaca", paper_only: true },
        entry.hostOrigin
      );
    }
  }

  if (typeof module !== "undefined" && module.exports) module.exports = { resolveModuleEntry };
