(function (root) {
  ...
  function resolveModuleEntry(search) {
    ...
    const allowed =
      standalone || !gatewayRequired || Boolean(token) || Boolean(hostOrigin);
    return {
  ...
  function applyShellChrome(entry) {
    ...
    if (modeBadge) {
      modeBadge.textContent = entry.mode === "embedded" ? "EMBEDDED" : "STANDALONE";
    }
    if (hostLink) {
    ...
    if (entry.embed && entry.hostOrigin && root.parent && root.parent !== root) {
      root.parent.postMessage(
        {
          type: "inneros.module.ready",
          module_id: "alpha.trading.alpaca",
          paper_only: true,
        },
        entry.hostOrigin
      );
